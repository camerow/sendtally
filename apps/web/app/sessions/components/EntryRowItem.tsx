 ...
 */
export function EntryRowItem({ entry }: { entry: JournalEntry }): React.ReactElement {
  ...
  return (
    <Link
      ...
      className="session-row session-row--entry"
      aria-label={[entryKindLabel(entry.kind), title, `${weekday} ${month} ${day}`].join(", ")}
    >
      ...
      <span className="session-row-badge">
        <span className={`entry-kind entry-kind--${entry.kind}`}>{entryKindLabel(entry.kind)}</span>
      </span>
