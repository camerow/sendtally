...
(cd "$root/packages/api" && pick '["TOKEN_KEY","CLERK_SECRET_KEY","CLERK_WEBHOOK_SIGNING_SECRET","STRAVA_CLIENT_ID","STRAVA_CLIENT_SECRET","STRAVA_WEBHOOK_VERIFY_TOKEN","REVENUECAT_SECRET_API_KEY","REVENUECAT_WEBHOOK_AUTH"]' | npx wrangler secret bulk --env "$env_name")
