...
- **Pull request previews are Cloudflare Workers Builds, and they target staging.** The connection lives on the `sendtally-web-staging` Worker (production branch `main`, "Builds for non-production branches" on) and reports as the `Workers Builds: sendtally-web-staging` check, which carries the preview URL. `sendtally-web-production` has no git connection - a build must never be able to reach live traffic.
  ...
  `preview_urls: true` in the staging env is necessary but was not sufficient: the flag reached the generated `build/server/wrangler.json` and the Worker still came back with `previews_enabled: false`, so the first build produced a version with no URL to open. It was flipped once through the API (`POST /accounts/:account/workers/scripts/sendtally-web-staging/subdomain` with `{"enabled": false, "previews_enabled": true}`) and sticks from there. Check that endpoint first if a build passes but the pull request comment carries only a logs link.
  ...
  Cloudflare's own pull request comment carries a logs link and no preview URL, so the `preview` job in `ci.yml` posts the URL itself. It derives the URL from the branch rather than from any build output, so the link appears immediately and starts working when the build lands.
